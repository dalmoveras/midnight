...
def connectDB():
    con = sqlite3.connect('output/midnight.db', timeout=30)
    return con
...
def searchDB(term, con):
    cur = con.cursor()
    query = "SELECT URL,Directory FROM Deepdata WHERE HTML LIKE \'%" + \
        term + "%\' ORDER BY URL;"
    cur.execute(query)
    results = cur.fetchall()
    return results
...
def searchFTS(term, con):
    cur = con.cursor()
    query = "SELECT URL,Directory FROM Deepsearch WHERE HTML MATCH \'" + \
        term + "\' ORDER BY rank;"
    cur.execute(query)
    results = cur.fetchall()
    return results
